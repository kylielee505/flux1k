...
def load_clip_weights(model, sd):
    ...
    sd = totoro.utils.clip_text_transformers_convert(sd, "cond_stage_model.model.", "cond_stage_model.transformer.")
    ...
    def __init__(self, sd=None, device=None, config=None, dtype=None):
        ...
        if config is None:
            if "decoder.mid.block_1.mix_factor" in sd:
                ...
                self.first_stage_model = totoro.taesd.taesd.TAESD()
...
def load_checkpoint(config_path=None, ckpt_path=None, output_vae=True, output_clip=True, embedding_directory=None, state_dict=None, config=None):
    ...
    if state_dict is None:
        state_dict = totoro.utils.load_torch_file(ckpt_path)

    class EmptyClass:
        pass

    model_config = totoro.supported_models_base.BASE({})
    ...
    if output_vae:
        vae_sd = totoro.utils.state_dict_prefix_replace(state_dict, {"first_stage_model.": ""}, filter_keys=True)
    ...
    return (totoro.model_patcher.ModelPatcher(model, load_device=model_management.get_torch_device(), offload_device=offload_device), clip, vae)
...
def load_checkpoint_guess_config(ckpt_path, output_vae=True, output_clip=True, output_clipvision=False, embedding_directory=None, output_model=True):
    ...
    parameters = totoro.utils.calculate_parameters(sd, "model.diffusion_model.")
    ...
    if output_model:
        model_patcher = totoro.model_patcher.ModelPatcher(model, load_device=load_device, offload_device=model_management.unet_offload_device(), current_device=inital_load_device)
