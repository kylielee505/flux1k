...
from totoro.model_sampling import EPS, V_PREDICTION, EDM, ModelSamplingDiscrete, ModelSamplingContinuousEDM, StableCascadeSampling
...
class BaseModel(torch.nn.Module):
    ...
    def memory_required(self, input_shape):
        if totoro.model_management.xformers_enabled() or totoro.model_management.pytorch_attention_flash_attention():
            ...
            return (area * totoro.model_management.dtype_size(dtype) / 50) * (1024 * 1024)
        else:
...
class SD3(BaseModel):
    ...
    def memory_required(self, input_shape):
        if totoro.model_management.xformers_enabled() or totoro.model_management.pytorch_attention_flash_attention():
            ...
            area = input_shape[0] * input_shape[2] * input_shape[3]
            return (area * totoro.model_management.dtype_size(dtype) * 0.012) * (1024 * 1024)
        else:
            area = input_shape[0] * input_shape[2] * input_shape[3]
